...
def match_score(keyword_tokens=list[str], text_tokens=list[str]) -> float:
    ...
    scores = math.log(1 + sum(single_match_score(keyword_token) for keyword_token in keyword_tokens))
...
def search_score(raw_data: pd.DataFrame, search_string: str) -> list:
    ...
    # Get title scores
    titles = [tokenize(s) for s in list(raw_data["job_title"])] # Transform titles into token lists
    ...
    description_scores = [math.log(1 + match_score(keywords, x)) for x in descriptions]
